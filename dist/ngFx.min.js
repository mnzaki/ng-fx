...
(window._gsQueue || (window._gsQueue = [])).push(function () {
  ...
  window._gsDefine('TweenMax', [
    ...
    var _slice = [].slice, TweenMax = function (target, duration, vars) {
      ...
      }, _tinyNum = 1e-10, _isSelector = TweenLite._internals.isSelector, _isArray = TweenLite._internals.isArray, p = TweenMax.prototype = TweenLite.to({}, 0.1, {}), _blankArray = [];
 ...
 */
  window._gsDefine('TimelineLite', [
    ...
    var TimelineLite = function (vars) {
      ...
      }, _tinyNum = 1e-10, _isSelector = TweenLite._internals.isSelector, _isArray = TweenLite._internals.isArray, _blankArray = [], _globals = window._gsDefine.globals, _copy = function (vars) {
        var copy = {}, p;
        for (p in vars) {
          copy[p] = vars[p];
        }
    ...
    p.addPause = function (position, callback, params, scope) {
      return this.call(_pauseCallback, [
        '{self}',
        callback,
        params,
        scope
 ...
 */
  (function () {
    var _RAD2DEG = 180 / Math.PI, _r1 = [], _r2 = [], _r3 = [], _corProps = {}, Segment = function (a, b, c, d) {
        ...
        var a = [], lengths = [], d = 0, total = 0, threshold = resolution - 1, segments = [], curLS = [],
 ...
 */
  window._gsDefine('plugins.CSSPlugin', [
    ...
    var _numExp = /(?:\d|\-\d|\.\d|\-\.\d)+/g, _relNumExp = /(?:\d|\-\d|\.\d|\-\.\d|\+=\d|\-=\d|\+=.\d|\-=\.\d)+/g, _valuesExp = /(?:\+=|\-=|\-|\b)[\d\-\.]+[a-zA-Z0-9]*(?:%|\b)/gi,
      //finds all the values that begin with numbers or += or -= and then a number. Includes suffixes. We use this to split complex values apart like "1px 5px 20px rgb(255,102,51)"
      _NaNExp = /[^\d\-\.]/g, _suffixExp = /(?:\d|\-|\+|=|#|\.)*/g, _opacityExp = /opacity *= *([^)]*)/, _opacityValExp = /opacity:([^;]*)/, _alphaFilterExp = /alpha\(opacity *=.+?\)/i, _rgbhslExp = /^(rgb|hsl)/, _capsExp = /([A-Z])/g, _camelExp = /-([a-z])/gi, _urlExp = /(^(?:url\(\"|url\())|(?:(\"\))$|\)$)/gi,
      //for pulling out urls from url(...) or url("...") strings (some browsers wrap urls in quotes, some don't when reporting things like backgroundImage)
      _camelFunc = function (s, g) {
        return g.toUpperCase();
      }, _horizExp = /(?:Left|Right|Width)/i, _ieGetMatrixExp = /(M11|M12|M21|M22)=[\d\-\.e]+/gi, _ieSetMatrixExp = /progid\:DXImageTransform\.Microsoft\.Matrix\(.+?\)/i, _commasOutsideParenExp = /,(?=[^\)]*(?:\(|$))/gi,
      //finds any commas that are not within parenthesis
      _DEG2RAD = Math.PI / 180, _RAD2DEG = 180 / Math.PI, _forcePT = {}, _doc = document, _tempDiv = _doc.createElement('div'), _tempImg = _doc.createElement('img'), _internals = CSSPlugin._internals = { _specialProps: _specialProps },
      //provides a hook to a few internal methods that we need to access from inside other plugins
      _agent = navigator.userAgent, _autoRound, _reqSafariFix,
      //we won't apply the Safari transform fix until we actually come across a tween that affects a transform property (to maintain best performance).
      _isSafari, _isFirefox,
      ...
      _parseComplex = CSSPlugin.parseComplex = function (t, p, b, e, clrs, dflt, pt, pr, plugin, setRatio) {
        ...
        for (i = 0; i < l; i++) {
          ...
          if (bn || bn === 0) {
            ...
            if (!bnums) {
              pt['xs' + pt.l] += pt.l ? ' ' + bv : bv;  //loop through all the numbers that are found and construct the extra values on the pt.
            } else {
    ...
    var _transformProps = 'scaleX,scaleY,scaleZ,x,y,z,skewX,skewY,rotation,rotationX,rotationY,perspective'.split(','), _transformProp = _checkPropPrefix('transform'),
      ...
      _getTransform = _internals.getTransform = function (t, cs, rec, parse) {
        ...
        var tm = rec ? t._gsTransform || new Transform() : new Transform(), invX = tm.scaleX < 0,
          //in order to interpret things properly, we need to know if the user applied a negative scaleX previously so that we can adjust the rotation and skewX accordingly. Otherwise, if we always interpret a flipped matrix as affecting scaleY and the user only wants to tween the scaleX on multiple sequential tweens, it would keep the negative scaleY without that being the user's intent.
          min = 0.00002, rnd = 100000, minAngle = 179.99, minPI = minAngle * _DEG2RAD, zOrigin = _supports3D ? parseFloat(_getStyle(t, _transformOriginProp, cs, false, '0 0 0').split(' ')[2]) || tm.zOrigin || 0 : 0, s, m, i, n, dec, scaleX, scaleY, rotation, skewX, difX, difY, difR, difS;
        if (_transformProp) {
    ...
    _registerComplexSpecialProp('backgroundPosition', {
      defaultValue: '0 0',
      parser: function (t, e, p, cssp, pt, plugin) {
        var bp = 'background-position', cs = _cs || _getComputedStyle(t, null), bs = this.format((cs ? _ieVers ? cs.getPropertyValue(bp + '-x') + ' ' + cs.getPropertyValue(bp + '-y') : cs.getPropertyValue(bp) : t.currentStyle.backgroundPositionX + ' ' + t.currentStyle.backgroundPositionY) || '0 0'),
                ...
                if (en || en === 0)
                  if (bn) {
                    ...
                    if (esfx === '%') {
                      ...
                      bn /= _convertToPixels(target, p, 1, 'em');  //otherwise convert to pixels.
                    } else if (esfx !== 'px') {
 ...
 */
  window._gsDefine.plugin({
    ...
    init: function (target, value, tween) {
      ...
      this._proxy = {};
      this._start = {};
      // we record start and end values exactly as they are in case they're strings (not numbers) - we need to be able to revert to them cleanly.
      this._end = {};
    ...
    set: function (ratio) {
      ...
      var props = this._overwriteProps, i = props.length, lookup = ratio !== 0 && ratio !== 1 ? this._proxy : ratio === this._endRatio ? this._end : this._start, p;
      while (--i > -1) {
        p = props[i];
...
(function (window) {
  ...
  var _namespace = function (ns) {
      ...
      return p;
    }, gs = _namespace('com.greensock'), _tinyNum = 1e-10, _slice = [].slice, _emptyFunc = function () {
    }, _isArray = function () {
 ...
 */
  var _baseParams = [
      ...
      1,
      1
    ], _blankArray = [], Ease = _class('easing.Ease', function (func, extraParams, type, power) {
      this._func = func;
      this._type = type || 0;
      this._power = power || 0;
...
(function (angular) {
  'use strict';
  angular.module('fx.animations.assist', []).factory('Assist', [
    '$filter',
    '$window',
    '$timeout',
    '$rootScope',
    function ($filter, $window, $timeout, $rootScope) {
      return {
        emit: function (element, animation, motion) {
          $rootScope.$broadcast(animation + ':' + motion);
        },
        parseClassList: function (element, option) {
          var ease, list = element[0].classList, results = {
              trigger: false,
              duration: 0.3,
              ease: $window.Back
            };
...
(function (angular, TweenMax, TimelineMax) {
  ...
  angular.module('fx.animations.create', ['fx.animations.assist']).factory('FadeAnimation', [
    '$timeout',
    '$window',
    'Assist',
    function ($timeout, $window, Assist) {
      return function (effect) {
        var inEffect = effect.enter, outEffect = effect.leave, outEffectLeave = effect.inverse || effect.leave, fx_type = effect.animation;
  ...
  ]).factory('BounceAnimation', [
    '$timeout',
    '$window',
    'Assist',
    function ($timeout, $window, Assist) {
      return function (effect) {
        var start = effect.first, mid = effect.mid, third = effect.third, end = effect.end, fx_type = effect.animation, startTime = 0.1;
        this.enter = function (element, done) {
          var options = Assist.parseClassList(element);
          options.motion = 'enter';
          options.animation = fx_type;
  ...
  ]).factory('RotateAnimation', [
    '$timeout',
    '$window',
    'Assist',
    function ($timeout, $window, Assist) {
      return function (effect) {
        var start = effect.start, end = effect.end, leaveEnd = effect.inverse, fx_type = effect.animation;
        this.enter = function (element, done) {
          var options = Assist.parseClassList(element);
          options.motion = 'enter';
          options.animation = fx_type;
    ...
    function ($timeout, $window, Assist) {
      return function (effect) {
        var start = effect.start, end = effect.end, fx_type = effect.animation;
        this.enter = function (element, done) {
          var options = Assist.parseClassList(element);
          options.motion = 'enter';
          options.animation = fx_type;
...
(function (angular) {
  'use strict';
  angular.module('fx.animations.bounces', ['fx.animations.create']).animation('.fx-bounce-normal', [
    'BounceAnimation',
...
(function (angular) {
  'use strict';
  angular.module('fx.animations.fades', ['fx.animations.create']).animation('.fx-fade-normal', [
    'FadeAnimation',
...
(function (angular) {
  'use strict';
  angular.module('fx.animations.rotations', ['fx.animations.create']).animation('.fx-rotate-counterclock', [
    'RotateAnimation',
  ...
  ]).animation('.fx-rotate-clock-left', [
    'RotateAnimation',
    function (RotateAnimation) {
      var effect = {
          start: {
  ...
  ]).animation('.fx-rotate-counterclock-right', [
    'RotateAnimation',
    function (RotateAnimation) {
      var effect = {
          start: {
  ...
  ]).animation('.fx-rotate-counterclock-up', [
    'RotateAnimation',
    function (RotateAnimation) {
      var effect = {
          start: {
...
(function (angular) {
  'use strict';
  angular.module('fx.animations.zooms', ['fx.animations.create']).animation('.fx-zoom-normal', [
    'ZoomAnimation',
...
(function (angular) {
  'use strict';
  var timeoutKey = '$$fxtimer';
  angular.module('fx.transitions.assist', []).factory('TransAssist', [
    '$timeout',
    function ($timeout) {
...
(function (angular, TLM) {
  ...
  angular.module('fx.transitions.create', [
    'fx.transitions.assist',
    'fx.animations.assist'
  ]).factory('SlideTransition', [
    'TransAssist',
    'Assist',
    function (TransAssist, Assist) {
      var slide;
      return function (effect) {
        if (effect.from) {
  ...
  ]).factory('RotationTransition', [
    'TransAssist',
    'Assist',
    '$compile',
    function (TransAssist, Assist, $compile) {
      var rotate;
      return function (effect) {
        this[effect.when] = function (el, done) {
          ...
          TransAssist.addTimer(el, effect.duration, done);
          rotate = new TLM();
          rotate.from(el, 1, effect.from).to(el, 1, effect.to);
          return function (cancel) {
            if (cancel) {
              TransAssist.removeTimer(el);
            }
  ...
  function cssMixin(el, z) {
    el.css('position', 'absolute');
    z ? z === 'leave' ? el.css('z-index', '9999') : el.css('z-index', '8888') : function () {
    };
  }
...
(function () {
  'use strict';
  angular.module('fx.transitions.view', []).directive('fxAnimate', [
    '$injector',
    function ($injector) {
      return {
...
(function (angular) {
  'use strict';
  angular.module('fx.transitions.rotations', ['fx.transitions.create']).animation('.rotate-out-right', [
    'RotationTransition',
...
(function (angular) {
  'use strict';
  angular.module('fx.transitions.scales', ['fx.transitions.create']).animation('.shrink-in', [
    'SlideTransition',
...
(function (angular) {
  'use strict';
  angular.module('fx.transitions.slides', ['fx.transitions.create']).animation('.slide-in-left', [
    'SlideTransition',
    function (SlideTransition) {
      var effect = {
  ...
  ]).animation('.slide-out-right-fade', [
    'SlideTransition',
    function (SlideTransition) {
      var effect = {
          to: {
...
(function (angular, TLM) {
  'use strict';
  angular.module('fx.transitions.specials', []).animation('.fx-fall-out', function () {
    // var effect = {
    //   from: {}
    // };
    return {
      leave: function (el, done) {
        ...
        page.to(el, { transform: 'rotateZ(0deg)' }).to(el, 0.1, { transform: 'rotateZ(10deg)' }).to(el, 0.3, { transform: 'rotateZ(17deg)' }).to(el, 0.5, { transform: 'rotateZ(15deg)' }).to(el, 0.2, { transform: 'translateY(100%) rotateZ(17deg)' });
      }
    };  // return new SlideTransition(effect);
  });
}(angular, TimelineMax));
